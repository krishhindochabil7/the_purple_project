...
BRIDGE_URL = "http://localhost:8001/claude/complete"

logger = logging.getLogger("jira_copilot.claude_llm")
...
class ClaudeLLM:
    def __init__(self, bridge_url: str = BRIDGE_URL, timeout_seconds: float = 120.0, retries: int = 2, workspace_path: str = ""):
    ...
    def _complete(self, prompt: str) -> str:
        ...
        for attempt in range(1, self.retries + 2):
            ...
            try:
                ...
                logger.info("claude_complete_success", extra={"attempt": attempt, "elapsed_ms": elapsed_ms})
                data = response.json()
                ...
                elapsed_ms = int((time.perf_counter() - started) * 1000)
                logger.warning(
                    "claude_complete_retry",
                    extra={"attempt": attempt, "elapsed_ms": elapsed_ms, "error": str(exc)},
                )
                last_error = exc
                if attempt <= self.retries:
                    time.sleep(0.5 * attempt)
        raise RuntimeError(f"Claude bridge request failed: {last_error}")
    ...
    def _json(self, prompt: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Claude response was not JSON: {content[:500]}")
    ...
    def _coerce_reasoning(data: dict[str, Any]) -> ReasoningResponse:
        ...
        if not data.get("decision") or not data.get("rationale"):
            raise ValueError("Claude reasoning response missing decision or rationale")
        return {
...
{reasoning}"""
        ...
        if not steps:
            raise ValueError("Claude plan response missing non-empty plan")
        return steps
...
{file_content}"""
        ...
        if missing:
            raise ValueError(f"Claude execution response missing required keys: {missing}")
        return {
            "file": data["file"],
            "before": data["before"],
            "after": data["after"],
...
{self._context_text(context_files)}"""
        ...
        if not isinstance(updates, list) or not updates:
            raise ValueError("Claude file update response missing non-empty updates array")
        normalized: list[dict[str, str]] = []
        for item in updates:
            if not isinstance(item, dict) or not isinstance(item.get("path"), str) or not isinstance(item.get("content"), str):
                raise ValueError("Each Claude update must include string path and content")
            normalized.append({"path": item["path"], "content": item["content"]})
        rationale = data.get("rationale")
        if not isinstance(rationale, str) or not rationale.strip():
            raise ValueError("Claude file update response missing rationale")
